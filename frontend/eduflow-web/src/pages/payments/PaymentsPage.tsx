  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <StartupBanner
        ...
        title="Moliyaviy Hisob-Kitob va To‘lovlar"
        description="O‘quvchilar kurs to‘lovlari, oilaviy va individual chegirmalar, qisman to‘lovlar, o‘qituvchi ulushi va markaz sof foydasi."
        icon={<CreditCard className="w-6 h-6" />}
        gradientTheme="emerald"
        metrics={[
          { label: 'Jami to‘lov qaydlari', value: `${payments.length} ta` },
          { label: 'Markaz sof foydasi', value: summary ? `${(summary.netProfit || 0).toLocaleString()} UZS` : '0 UZS' },
        ]}
        actions={
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex bg-white/90 dark:bg-slate-900/90 p-1.5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs gap-1">
          ...
          >
            📉 Markaz Xarajatlari ({expenses.length})
          </button>
        </div>
        ...
        {activeTab === 'payments' && (
          <div className="flex bg-white/90 dark:bg-slate-900/90 p-1 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs gap-1 overflow-x-auto">
            {[
              { id: undefined, label: '📑 Barchasi' },
              { id: 5, label: '⏳ Qisman (Kutilmoqda)' },
              { id: 3, label: '⚠️ Qarzdorlik' },
              { id: 1, label: '🕒 To‘lov kutilmoqda' },
              { id: 2, label: '✅ To‘langan' },
            ].map((tab) => (
              <button
                key={tab.label}
                onClick={() => {
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                    <th className="py-3.5 px-5">O‘quvchi & Guruh</th>
                    <th className="py-3.5 px-4">Kurs Narxi & Chegirma</th>
                    <th className="py-3.5 px-4">To‘langan / Qarz</th>
                    <th className="py-3.5 px-4">O‘qituvchi Ulushi</th>
